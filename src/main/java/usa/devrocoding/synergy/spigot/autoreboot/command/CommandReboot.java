package usa.devrocoding.synergy.spigot.autoreboot.command;
    ...
    public void execute(SynergyUser synergyUser, Player player, String command, String[] args) {
        if (args.length == 1){
            ...
            try{
                ...
                synergyUser.error("Wrong format! " + args[0] + " is not a number.");
                return;
            }
            synergyUser.info("Starting the reboot sequence for you...");
            Core.getPlugin().getAutoRebootManager().rebootServer(SynergyPeriod.SECOND.getCustom(seconds), "Manual Restart");
        }else{
