...
import usa.devrocoding.synergy.spigot.assets.gui.WarpHomeGUI;
...
public class CommandWarp extends SynergyCommand {
    ...
    public CommandWarp(Core plugin) {
        super(plugin, Rank.NONE, "Synergy's Warp Command", false,"warp");
    }
    ...
    public void execute(SynergyUser synergyUser, Player player, String command, String[] args) {
        if (args.length > 0 && args.length < 3){
            if (args[0].equalsIgnoreCase("create")){
                if (synergyUser.hasPermission("warp.create")){
                    try{
            ...
            if (synergyUser.hasPermission("warp.gui")) {
                new WarpHomeGUI(getPlugin()).open(player);
            }
        }
    }
